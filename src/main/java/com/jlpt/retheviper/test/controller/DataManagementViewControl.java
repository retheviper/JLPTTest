...
public class DataManagementViewControl implements Initializable {
	...
	private void setRightTableValue() { // 왼쪽 과목 콤보박스 선택시 테이블 변화
		switch (this.subject_left.getValue()) { // TODO
		case "언어지식(문자·어휘)":
			this.problemList = FXCollections.observableArrayList(this.service.getVList());
			break;
		case "언어지식(문법)":
			this.problemList = FXCollections.observableArrayList(this.service.getGList());
			break;
		case "독해":
			this.problemList = FXCollections.observableArrayList(this.service.getRList());
			break;
		case "청해":
			this.problemList = FXCollections.observableArrayList(this.service.getLList());
			break;
		}
		resetLeftTable();
	}
	...
	private void registProblem() { // 문제 등록
		if (checkForm()) {
			...
			final Problem problem = Problem.builder().subject(Subject.getTypeByValues(this.subject_right.getValue()))
					...
					.mp3Source(this.mp3File != null ? this.mp3File.getName() : null)
					.build();

			if (this.service.registProblem(problem)) {
				refreshBothTable();
	...
	private void modifyProblem() { // 수정 버튼 클릭 시 테이블에서 대상을 선택하고 내용을 덮어씀
		if (checkForm() && checkTableItemSelected()) {
			...
			final Problem problem = Problem.builder().subject(Subject.valueOf(this.subject_right.getValue()))
					.pNumber(this.number_right.getText())
					.subNumber(this.subNumber_right.getText())
					.passage(this.passage_right.getText())
					.imgSource(this.imgSource)
					...
					.forthChoice(this.forthChoice_right.getText())
					.answer(convertAnswerToInt(this.answer_right.getValue()))
					.mp3Source(this.mp3File.getName())
					.build();
			service.modifyProblem(problem); // TODO
			refreshBothTable();
			clearInputForm();
